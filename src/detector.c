...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, char* chart_png)
{
...
#ifdef OPENCV
        draw_train_loss(img, img_size, avg_loss, max_img_loss, i, net.max_batches, mean_average_precision, draw_precision, "mAP%", dont_show, mjpeg_port, chart_png);
#endif    // OPENCV
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net)
{
    ...
    for (rank = 0; rank < detections_count; ++rank) {
        ...
        for (i = 0; i < classes; ++i)
        {
            ...
            if (rank == (detections_count - 1) && detection_per_class_count[i] != (tp + fp)) {    // check for last rank
                    fprintf(stderr, " class_id: %d - detections = %d, tp+fp = %d, tp = %d, fp = %d \n", i, detection_per_class_count[i], tp+fp, tp, fp);
            }
        }
    }
    ...
    for (i = 0; i < classes; ++i) {
        ...
        fprintf(stderr, "class_id = %d, name = %s, ap = %2.2f%%   \t (TP = %d, FP = %d) \n",
    ...
    printf(stderr, " for conf_thresh = %0.2f, TP = %d, FP = %d, FN = %d, average IoU = %2.2f %% \n",
        thresh_calc_avg_iou, tp_for_thresh, fp_for_thresh, unique_truth_count - tp_for_thresh, avg_iou * 100);

    mean_average_precision = mean_average_precision / classes;
    fprintf(stderr, "\n IoU threshold = %2.0f %%, ", iou_thresh * 100);
    if (map_points) fprintf(stderr, "used %d Recall-points \n", map_points);
    else fprintf(stderr, "used Area-Under-Curve for each unique Recall \n");
    ...
    fprintf(stderr, "Total Detection Time: %f Seconds\n", (double)(time(0) - start));
    fprintf(stderr, "\nSet -points flag:\n");
    fprintf(stderr, " `-points 101` for MS COCO \n");
    fprintf(stderr, " `-points 11` for PascalVOC 2007 (uncomment `difficult` in voc.data) \n");
    fprintf(stderr, " `-points 0` (AUC) for ImageNet, PascalVOC 2010-2012, your custom dataset\n");
...
void calc_anchors(char *datacfg, int num_of_clusters, int width, int height, int show)
{
    ...
    for (i = 0; i < number_of_boxes; ++i) {
        ...
        if (best_iou > 1 || best_iou < 0) { // || box_w > width || box_h > height) {
            fprintf(stderr, " Wrong label: i = %d, box_w = %f, box_h = %f, anchor_w = %f, anchor_h = %f, iou = %f \n",
...
void run_detector(int argc, char **argv)
{
    ...
    //generate a filename for chart that doesnt exist..
    char chart_png[100]; 
    int file_seq = 0;
    int file_exists = 0;
    do 
    {
        file_exists = 0;
        file_seq++;
        sprintf(chart_png, "chart_%d.png\0", file_seq);
        FILE *infile = fopen(chart_png, "r");
        if (infile != NULL)
        {
            file_exists = 1;
            fclose(infile);
    ...
    else if (0 == strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, mjpeg_port, show_imgs, chart_png);
